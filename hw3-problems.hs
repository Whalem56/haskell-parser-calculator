...
import Control.Monad       (Monad, MonadPlus, mzero, mplus, void)
import Data.Char
import Data.List
...
pReturn :: a -> Parser a
--pReturn = undefined
pReturn val = MkParser $ \pState -> (pState, ParseOK val)
...
--pBind = undefined
pBind (MkParser parseFunc) f = MkParser $ \pState ->
  let firstRes = parseFunc pState 
  in case firstRes of 
    (pState', ParseError err) -> (pState', ParseError err)
    (pState', ParseOK val) -> let (MkParser parseFunc') = f val
                              in parseFunc' pState'
...
pZero :: Parser a
--pZero = undefined
pZero = MkParser $ \pState -> (pState, ParseError (MkPError {erOffset = stOffset pState, found = Nothing, expected = []}))
...
--pPlus = undefined
pPlus (MkParser parseFunc) (MkParser parseFunc') = MkParser $ \pState ->
  let firstRes = parseFunc pState
  in case firstRes of 
    (pState1, ParseOK val1) -> (pState1, ParseOK val1)
    (pState1, ParseError err1) -> let secondRes = parseFunc' pState
                                  in case secondRes of  
                                    (pState2, ParseOK val2) -> (pState2, ParseOK val2)
                                    (pState2, ParseError err2) -> let (pState', err') = mergeErrors (pState1, err1) (pState2, err2)
                                                                  in (pState', ParseError err')
...
token :: (Char -> Bool)
      ...
      -> Parser Char
--token = undefined
token pred expected = MkParser $ \pState ->
  let (MkPState strToParse offset) = pState
  in
    if null strToParse
      then (MkPState strToParse offset, ParseError (MkPError offset (Just EndOfInput) []))
      else 
        let char    = head strToParse
            predRes = pred char
        in  (if predRes then
          (MkPState (drop 1 strToParse) (offset + 1), ParseOK char) else
          (MkPState strToParse offset,
           ParseError (MkPError offset (Just (Chunk [char])) expected)))
...
--single c = undefined
single char = token (char ==) [Chunk [char]]
...
--eof = undefined
eof = MkParser $ \pState ->
  let (MkPState strToParse _) = pState
  in if null strToParse
      then (pState, ParseOK ())
      else (pState, ParseError (MkPError {erOffset = stOffset pState, found = Just (Chunk [head strToParse]), expected = [EndOfInput]}))
...
--
-- (Hint: use optional to check if the parser succeeds. If it doesn't succeed, return [].
...
endBy :: MonadPlus m
      ...
      -> m [a]
--endBy = undefined
endBy parser sepParse = do
  first <- optional parser
  case first of 
    Nothing -> return []
    Just r -> do 
      second <- optional sepParse
      case second of
        Nothing -> return []
        Just _ -> do
          rest <- endBy parser sepParse 
          return (r : rest)
...
sepBy parser sepParse = do
  first <- optional parser
  case first of
    Nothing -> return []
    Just r  -> do
      rest <- optional (sepByHelper parser sepParse)
      case rest of 
        Nothing         -> return [r]
        Just listResult -> return (r : listResult)

sepByHelper :: MonadPlus m
      => m a
      -> m sep
      -> m [a]
sepByHelper parser sepParse = do
  sepParse
  result <- parser
  rest   <- optional (sepByHelper parser sepParse)
  case rest of
    Nothing         -> return [result]
    Just listResult -> return (result : listResult)
...
symbol :: String
       -> Parser String
--symbol = undefined
symbol input = chunk input <* optSpaces
...
keyword :: String
        -> Parser String
--keyword = undefined
keyword input = chunk input <* spaces
...
parseSimpleLLit' :: Parser [Int]
--parseSimpleLLit' = undefined
parseSimpleLLit' = between (symbol "[") (symbol "]") (sepBy number (many (oneOf ", ")))
...
parseSimpleLExpr' :: Parser SimpleLExpr
--parseSimpleLExpr' = undefined
parseSimpleLExpr' = do
  ints <- parseSimpleLLit
  some (oneOf "~ ")
  SimpleLConcat ints <$> parseSimpleLExpr'
  <|> do SimpleLSingle <$> parseSimpleLLit
...
parseAExpr' =   do
              ...
              <|>
                do
                   ...
                   aExpr1 <- parseAExpr
                   keyword "else"
                   IfThenElse bExpr aExpr1 <$> parseAExpr
...
parseBExpr' = BConst <$> boolean
              <|> do 
                aExpr1 <- parseAExpr
                symbol "=="
                BEq aExpr1 <$> parseAExpr
              <|> do 
                aExpr1 <- parseAExpr
                symbol "<"
                BLt aExpr1 <$> parseAExpr
              <|> do 
                aExpr1 <- parseAExpr
                symbol ">"
                BGt aExpr1 <$> parseAExpr
...
evalT :: Term
      -> Int
--evalT = undefined
evalT (FSingle fact) = evalF fact
evalT (Mult fact term) = evalF fact * evalT term

evalF :: Fact
      -> Int
--evalF = undefined
evalF (AConst int) = int
evalF (ASingle aexp) = evalA aexp

evalB :: BExpr
      -> Bool
--evalB = undefined
evalB (BConst bool) = bool
evalB (BEq aexp1 aexp2) = evalA aexp1 == evalA aexp2
evalB (BLt aexp1 aexp2) = evalA aexp1 < evalA aexp2
evalB (BGt aexp1 aexp2) = evalA aexp2 > evalA aexp2
...
--parseIO = undefined
parseIO parser = do
  putStrLn "Give me something to compute:"
  input <- getLine
  if input == "q"
    then return ()
    else do 
